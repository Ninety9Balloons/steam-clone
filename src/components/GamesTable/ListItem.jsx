export default function ListItem({
  ...
  return (
    <li
      className="group h-[70px] w-full flex  justify-between items-center bg-black/20 cursor-pointer transition-all
      hover:bg-gradient-to-r hover:from-[#c6e6f8] hover:to-[#84bcd3] hover:text-black"
    >
      {/* Left side information */}
      <div className="flex w-[70%] md:w-full max-w-[80%] h-full">
        {/* Image */}
        <img src={image} className="w-1/3 object-cover h-full" />

        {/* Details */}
        <div className="flex flex-col h-full justify-around text-[#c7d5e0] group-hover:text-black ml-3 text-sm overflow-hidden">
          {/* Title */}
          <h3 className="truncate">{title}</h3>
          {/* Available Systems */}
          <div className="flex opacity-30 group-hover:opacity-100">
            {platformsArray.map((item, i) => {
              if (item === "WIN") {
                return (
                  <span key={i}>
                    <img src="icon_platform_win.png" alt="Windows Icon" />
                  </span>
                );
              }
              if (item === "MAC") {
                return (
                  <span key={i}>
                    <img src="icon_platform_mac.png" alt="Mac Icon" />
                  </span>
                );
              }
              if (item === "LNX") {
                return (
                  <span key={i}>
                    <img src="icon_platform_linux.png" alt="Linux Icon" />
                  </span>
                );
              }
            })}
          ...
          {/* Tags */}
          <div className="text-xs flex text-[#384959] truncate">
      ...
      {/* Right side pricing */}
      <div className="flex items-center justify-between gap-1 md:gap-8 min-w-[20%]">
        {/* Discount */}
        {discount !== null ? (
          <div className="bg-[#4c6b22] p-1 h-[28px]">
        ...
        <div className="text-right mr-2 flex justify-end w-full">
          {discount !== null ? (
            <div className="flex flex-col justify-center">
              ...
              {/* Current Price */}
              <div className="p-0">
                <p className="text-[#beee11] group-hover:text-dark-blue text-sm">
            ...
            <div className="w-full flex justify-end">
              {formattedPrice === 0 ? (
                <p className="text-white group-hover:text-dark-blue text-right text-xs">
                  Free to Play
                </p>
              ) : (
                <p className="text-white group-hover:text-dark-blue text-right text-sm">
                  ${formattedPrice}
                </p>
              )}
            </div>
