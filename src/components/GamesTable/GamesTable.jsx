    ...
    <div className="flex flex-col w-full my-10">
      ...
      <ul className="text-[#4f94bc] items-center flex gap-1 ml-2">
        <li
          ...
          className={`cursor-pointer py-[1px] px-2 pb-1 hover:text-white transition-all text-sm rounded-t-sm transition-all ${
            activeTab === "newTrending" ? activeStyle : ""
          }`}
        >
          New & Trending
        </li>
        <li
          onClick={() => {
            setGameList(topRated);
            setActiveTab("topRated");
          }}
          className={`cursor-pointer py-[1px] px-2 pb-1 hover:text-white transition-all text-sm rounded-t-sm transition-all ${
            activeTab === "topRated" ? activeStyle : ""
          }`}
        >
          Top Rated
        </li>
        <li
          onClick={() => {
            setGameList(specials);
            setActiveTab("specials");
          }}
          className={`cursor-pointer py-[1px] px-2 pb-1 hover:text-white transition-all text-sm rounded-t-sm transition-all ${
            activeTab === "specials" ? activeStyle : ""
          }`}
        >
          Specials
        </li>
      ...
      <div className="flex justify-end gap-1 items-center bg-black/20 mx-2 p-1">
        ...
        <button className="text-white px-2 py-1 border border-white uppercase text-xs rounded-sm hover:shadow-[0_0_5px_0_#fff] transition-all">
          New Releases
        </button>
      </div>
    </div>
